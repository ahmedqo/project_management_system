...
@section('content')
    ...
    <div class="grid grid-rows-1 grid-cols-1 gap-4">
        <div class="w-full bg-white p-4 rounded-lg ">
            <form action="{{ route('actions.terminations.store') }}" method="POST" class="w-full flex flex-col gap-4">
                @csrf
                        ...
                        <div class="relative">
                            <select x-select id="reason" placeholder="Reason" name="reason">
                                @foreach (System::reasons() as $reason)
                                    <option x-ucfirst value="{{ $reason }}">{{ $reason }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="w-full">
                        ...
                        <div class="relative">
                            <select x-select id="status" placeholder="Status" name="status">
                                <option x-ucfirst value="1">On</option>
                                <option x-ucfirst value="0">Off</option>
                            </select>
                        </div>
                    </div>
                </div>
