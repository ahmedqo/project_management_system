...
@section('content')
    ...
    <div class="grid grid-rows-1 grid-cols-1 gap-4">
        <div class="w-full bg-white p-4 rounded-lg ">
            <form action="{{ route('actions.terminations.update', $data->id) }}" method="POST"
                ...
                <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-2 gap-4">
                    <div class="w-full">
                        <label x-ucfirst for="reason" class="block text-sm font-black text-gray-900 mb-1">Reason</label>
                        <div class="relative">
                            <select x-select id="reason" placeholder="Reason" name="reason">
                                @foreach (System::reasons() as $reason)
                                    <option x-ucfirst value="{{ $reason }}"
                                        @if ($data->reason == $reason) selected @endif>{{ $reason }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="w-full">
                        ...
                        <div class="relative">
                            <select x-select id="status" placeholder="Status" name="status">
                                <option x-ucfirst value="1" @if ($data->employee()->first()->status == 1) selected @endif>On
                                </option>
                                <option x-ucfirst value="0" @if ($data->employee()->first()->status == 0) selected @endif>Off
                                </option>
                            </select>
                        </div>
                    </div>
