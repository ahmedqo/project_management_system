...
@section('content')
    <div class="w-full bg-white rounded-lg -mt-12 overflow-hidden">
        <div class="p-4 flex items-center justify-between gap-4">
            ...
            <div class="w-max flex items-center gap-2">
                ...
                <a href="{{ route('views.employees.create') }}"
    ...
    <div class="w-full">
        <div class="w-full bg-white p-4 rounded-lg ">
            <table x-table x-hide="5" x-name="employees">
                <thead>
                    <tr>
                        <td>
                            full name
                ...
                <tbody>
                    @foreach ($data as $row)
                        <tr>
                            ...
                            <td x-ucfirst>
                                {{ $row->department()->first()->name }}
                            </td>
                            <td x-ucfirst>
                                {{ $row->designation()->first()->name }}
                            </td>
                            <td>
                                {{ Carbon::parse($row->created_at)->diffForHumans() }}
                            </td>
                            <td>
                                <span class="hidden">{{ $row->status }}</span>
                                <div class="w-full h-full flex items-center justify-center">
                                    <label class="relative flex items-center gap-2">
                                        <input type="checkbox" class="sr-only peer" {{ $row->status ? 'checked' : '' }}
                                            onchange="change(`{{ route('actions.employees.status', ['id' => $row->id, 'status' => $row->status == 1 ? 0 : 1]) }}`)">
                                        <div
                                            class="w-12 h-6 cursor-pointer rounded-full bg-gray-50 border border-gray-300 peer peer-checked:after:translate-x-full after:border after:border-gray-300 after:content-[''] after:absolute after:top-1/2 after:-translate-y-1/2 after:left-0 after:bg-white after:rounded-full after:h-6 after:w-6 after:shadow-sm after:transition-all peer-checked:bg-primary">
                                        </div>
                                    </label>
                                </div>
                            </td>
                            <td>
                                <div class="w-full h-full flex items-center justify-center gap-2">
                                    <a href="{{ route('views.employees.summary', $row->id) }}"
                                    ...
                                    <a href="{{ route('views.employees.edit', $row->id) }}"
                                    ...
                                    <a href="{{ route('actions.employees.destroy', $row->id) }}"
...
@section('print')
    <h1 class="text-gray-900 font-black text-2xl text-center mb-16">Employees List</h1>
