    ...
    <div class="w-full bg-white rounded-lg -mt-12 overflow-hidden">
        <div class="p-4 flex items-center justify-between gap-4">
            ...
            <div class="w-max flex items-center gap-2">
                ...
                <a href="{{ route('views.leaves.create', ['employee' => $employee->id]) }}"
            ...
            <tbody>
                @foreach ($data as $row)
                    <tr>
                        ...
                        <td>
                            {{ abs(round((strtotime($row->endDate) - strtotime($row->startDate)) / 86400)) }}
                        </td>
                        <td>
                        ...
                        <td>
                            <div class="w-full h-full flex items-center justify-center gap-2">
                                <a href="{{ route('views.leaves.edit', $row->id) }}"
                                ...
                                <a href="{{ route('actions.leaves.destroy', $row->id) }}"
            ...
            <tbody>
                @forelse($data as $row)
                    <tr>
                        ...
                        <td class="px-4 py-2">
                            {{ abs(round((strtotime($row->endDate) - strtotime($row->startDate)) / 86400)) }} days
                        </td>
                        <td class="px-4 py-2">
                            {{ $row->startDate }}
                        </td>
