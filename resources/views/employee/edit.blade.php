...
@section('content')
    ...
    <div class="grid grid-rows-1 grid-cols-1 gap-4">
        <div class="w-full bg-white p-4 rounded-lg ">
            <form action="{{ route('actions.employees.update', $data->id) }}" method="POST"
                        ...
                        <div class="relative">
                            <select x-select id="department" placeholder="Department" name="department">
                                @foreach ($departments as $department)
                                    <option x-ucfirst value="{{ $department->id }}"
                                        @if ($data->department == $department->id) selected @endif>
                        ...
                        <div class="relative">
                            <select x-select id="designation" placeholder="Designation" name="designation">
                                @foreach ($designations as $designation)
                                    <option x-ucfirst value="{{ $designation->id }}"
                                        @if ($data->designation == $designation->id) selected @endif>
                            ...
                            <select x-select id="insurance" placeholder="Insurance" name="insurance">
                                @foreach ($insurances as $insurance)
                                    <option x-ucfirst value="{{ $insurance->id }}"
                                        @if ($data->insurance == $insurance->id) selected @endif>
