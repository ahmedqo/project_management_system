...
@section('content')
    ...
    <div class="grid grid-rows-1 grid-cols-1 gap-4">
        <div class="w-full bg-white p-4 rounded-lg ">
            <form action="{{ route('actions.tasks.store') }}" method="POST" class="w-full flex flex-col gap-4">
                @csrf
                <div class="w-full">
                        ...
                        <div class="relative">
                            <select x-select id="priority" placeholder="Priority" name="priority">
                                @foreach (System::priorities() as $priority)
                                    <option x-ucfirst value="{{ $priority }}">{{ $priority }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
@section('script')
    <script>
        if (queries().project) {
            ...
            Array.from(project.options).forEach((option, i) => {
                if (option.value === queries().project) option.setAttribute('selected', '');
            });
        }
