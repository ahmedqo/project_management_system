...
@section('content')
    ...
    <div class="grid grid-rows-1 grid-cols-1 gap-4">
        <div class="w-full bg-white p-4 rounded-lg ">
            <form action="{{ route('actions.tasks.update', $data->id) }}" method="POST" class="w-full flex flex-col gap-4">
                @csrf
                <div class="w-full">
                ...
                <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-2 gap-4">
                    <div class="w-full">
                        <label x-ucfirst for="project" class="block text-sm font-black text-gray-900 mb-1">Project</label>
                        <div class="relative">
                            <select x-select id="project" placeholder="Project" name="project">
                                @foreach ($projects as $project)
                                    <option x-ucfirst value="{{ $project->id }}"
                                        @if ($data->project == $project->id) selected @endif>
                                        {{ $project->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="w-full">
                        ...
                        <div class="relative">
                            <select x-select id="priority" placeholder="Priority" name="priority">
                                @foreach (System::priorities() as $priority)
                                    <option x-ucfirst value="{{ $priority }}"
                                        @if ($data->priority == $priority) selected @endif>{{ $priority }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
