...
@section('content')
    ...
    <div class="grid grid-rows-1 grid-cols-1 gap-4">
        <div class="w-full bg-white p-4 rounded-lg ">
            <form action="{{ route('actions.accounts.store') }}" method="POST" class="w-full flex flex-col gap-4">
                @csrf
                    ...
                    <div class="w-full lg:flex-1">
                        ...
                        <div class="relative">
                            <select x-select id="type" placeholder="Type" name="type">
                                <option x-ucfirst value="company" selected>Company</option>
                                <option x-ucfirst value="employee">Employee</option>
                                <option x-ucfirst value="client">Client</option>
                            </select>
                        </div>
                    </div>
...
@section('script')
    <script>
        if (queries().employee) {
            document.querySelector("#type").options[1].setAttribute('selected', '');
        ...
        if (queries().client) {
            document.querySelector("#type").options[2].setAttribute('selected', '');
