...
@section('content')
    ...
    <div class="grid grid-rows-1 grid-cols-1 gap-4">
        <div class="w-full bg-white p-4 rounded-lg ">
            <form action="{{ route('actions.expenses.store') }}" method="POST" class="w-full flex flex-col gap-4">
                @csrf
                        ...
                        <div class="relative">
                            <select x-select id="type" placeholder="Type" name="type">
                                @foreach (System::expenses() as $type)
                                    <option x-ucfirst value="{{ $type }}">{{ $type }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-2 gap-4">
                    <div class="w-full">
                        ...
                        <input x-date id="date" placeholder="Date" name="date" />
                    </div>
                    <div class="w-full">
