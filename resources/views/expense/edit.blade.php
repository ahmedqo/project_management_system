...
@section('content')
    ...
    <div class="grid grid-rows-1 grid-cols-1 gap-4">
        <div class="w-full bg-white p-4 rounded-lg ">
            <form action="{{ route('actions.expenses.update', $data->id) }}" method="POST" class="w-full flex flex-col gap-4">
                @csrf
                    ...
                    <div class="w-full">
                        ...
                        <div class="relative">
                            <select x-select id="type" placeholder="Type" name="type">
                                @foreach (System::expenses() as $type)
                                    <option x-ucfirst value="{{ $type }}"
                                        @if ($data->type == $type) selected @endif>{{ $type }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-2 gap-4">
                    <div class="w-full">
                        ...
                        <input x-date id="date" placeholder="Date" name="date" value="{{ $data->date }}" />
                    </div>
                    <div class="w-full">
                        <label x-ucfirst for="amount" class="block text-sm font-black text-gray-900 mb-1">Amount</label>
                        <input type="number" id="amount" placeholder="Amount" name="amount" value="{{ $data->amount }}"
                ...
                <div class="w-full">
                    ...
                    <div class="relative">
                        <select x-select id="status" placeholder="Status" name="status">
                            @foreach (System::stages('status') as $type)
                                <option x-ucfirst value="{{ $type }}"
                                    @if ($data->status == $type) selected @endif>{{ $type }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="w-full">
                    ...
                    <div class="relative">
                        <textarea id="note" placeholder="Note" name="note"
                            class="appearance-none bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-md block w-full p-2 focus:outline-1 outline-primary">{{ $data->note }}</textarea>
                    </div>
                </div>
                <div class="w-full">
                    <button type="submit"
