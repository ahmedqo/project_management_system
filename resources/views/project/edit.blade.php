...
@section('title', 'Edit Project #' . $data->id)
...
@section('content')
    ...
    <div class="grid grid-rows-1 grid-cols-1 gap-4">
        <div class="w-full bg-white p-4 rounded-lg ">
            <form action="{{ route('actions.projects.update', $data->id) }}" method="POST" class="w-full flex flex-col gap-4">
                @csrf
                <div class="w-full">
                            ...
                            <select x-select id="manager" placeholder="Manager" name="manager">
                                @foreach ($employees as $employee)
                                    <option x-ucfirst value="{{ $employee->id }}"
                                        @if ($employee->id == $data->manager) selected @endif>
                    ...
                    <div class="w-full">
                        ...
                        <div class="relative">
                            <select x-select multiple id="employees" placeholder="Employees" name="employees[]">
                                @foreach ($employees as $employee)
                                    <option x-ucfirst value="{{ $employee->id }}"
                                        @if ($projectEmployees->contains('employee', $employee->id)) selected @endif>
                                        {{ $employee->firstName }} {{ $employee->lastName }}
                                    </option>
                                @endforeach
                            </select>
