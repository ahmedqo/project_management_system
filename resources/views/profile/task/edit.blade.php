...
@section('title', 'Edit Task #' . $data->id)
...
@section('content')
    ...
    <div class="grid grid-rows-1 grid-cols-1 gap-4">
        <div class="w-full bg-white p-4 rounded-lg ">
            <form action="{{ route('actions.profile.tasks.update', $data->id) }}" method="POST"
                ...
                <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div class="w-full">
                        <label x-ucfirst for="priority" class="block text-sm font-black text-gray-900 mb-1">Priority</label>
                        <div class="relative">
                            <select x-select id="priority" placeholder="Priority" name="priority">
                                <option x-ucfirst value="low" @if ($data->priority == 'low') selected @endif>Low
                                </option>
                                <option x-ucfirst value="medium" @if ($data->priority == 'medium') selected @endif>Medium
                                </option>
                                <option x-ucfirst value="high" @if ($data->priority == 'high') selected @endif>High
                                </option>
                                <option x-ucfirst value="critical" @if ($data->priority == 'critical') selected @endif>
                                    Critical</option>
                            </select>
                        </div>
                    </div>
                ...
                <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-2 gap-4">
                    <div class="w-full">
                        ...
                        <div class="relative">
                            <select x-select multiple id="employee" placeholder="Employees" name="employees[]">
                                @foreach ($employees as $employee)
                                    <option x-ucfirst value="{{ $employee->id }}"
                                        @if ($taskEmployees->contains('employee', $employee->id)) selected @endif>
                    ...
                    <div class="w-full">
                        ...
                        <div class="relative">
                            <select x-select id="status" placeholder="Status" name="status">
                                <option x-ucfirst value="pending" @if ($data->status == 'pending') selected @endif>Pending
                                </option>
                                <option x-ucfirst value="ready" @if ($data->status == 'ready') selected @endif>Ready
                                </option>
                                <option x-ucfirst value="progress" @if ($data->status == 'progress') selected @endif>
                                    Progress</option>
                                <option x-ucfirst value="closed" @if ($data->status == 'closed') selected @endif>Closed
                                </option>
                                <option x-ucfirst value="hold" @if ($data->status == 'hold') selected @endif>Hold
                                </option>
                            </select>
                        </div>
                    </div>
                ...
                <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div class="w-full">
                        <label x-ucfirst for="duration" class="block text-sm font-black text-gray-900 mb-1">Duration
                            (hours)</label>
                        <div class="relative">
                            <input id="duration" type="number" placeholder="Duration (hours)" name="duration"
                                value="{{ $data->duration }}" step="0.05"
                                class="appearance-none bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-md block w-full p-2 focus:outline-1 outline-primary" />
                        </div>
                    </div>
                </div>
