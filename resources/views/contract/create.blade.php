...
@section('content')
    ...
    <div class="grid grid-rows-1 grid-cols-1 gap-4">
        <div class="w-full bg-white p-4 rounded-lg ">
            <form action="{{ route('actions.contracts.store') }}" method="POST" class="w-full flex flex-col gap-4">
                @csrf
                    ...
                    <div class="w-full">
                        ...
                        <div class="relative">
                            <select x-select id="type" placeholder="Type" name="type">
                                @foreach (System::contracts() as $contractType)
                                    <option x-ucfirst value="{{ $contractType }}">
                                        {{ $contractType }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div class="w-full">
                        ...
                        <input x-date id="endDate" placeholder="End date" name="endDate" />
                    </div>
                </div>
                <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-2 gap-4">
                    <div class="w-full">
                        <label x-ucfirst for="policies" class="block text-sm font-black text-gray-900 mb-1">Policies</label>
                        <div class="relative">
                            <select x-select multiple id="policies" placeholder="Policies" name="policies[]">
                                @foreach ($policies as $policy)
                                    <option x-ucfirst value="{{ $policy->id }}">
                                        {{ $policy->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="w-full">
                        <label x-ucfirst for="probation" class="block text-sm font-black text-gray-900 mb-1">Probation
                            (days)</label>
                        <div class="relative">
                            <input id="probation" type="number" placeholder="Probation (days)" name="probation"
                ...
                <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-2 gap-4">
                    <div class="w-full">
                        <label x-ucfirst for="salary" class="block text-sm font-black text-gray-900 mb-1">Salary</label>
                        <div class="relative">
                            <input id="salary" type="number" placeholder="Salary" name="salary"
                                class="appearance-none bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-md block w-full p-2 focus:outline-1 outline-primary" />
                        </div>
                    </div>
                    <div class="w-full">
                        <label x-ucfirst for="compensation"
                            class="block text-sm font-black text-gray-900 mb-1">Compensation</label>
                        <div class="relative">
                            <select x-select id="compensation" placeholder="Compensation" name="compensation">
                                @foreach (System::compensations() as $compensation)
                                    <option x-ucfirst value="{{ $compensation }}">{{ $compensation }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="w-full">
                    <button type="submit"
                        class="appearance-none lg:w-max ml-auto text-md flex items-center justify-center rounded-md font-semibold w-full p-2 px-4 text-white outline-none bg-primary hover:bg-light focus:bg-light shadow-sm">
