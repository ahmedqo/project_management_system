...
@section('content')
    <div class="w-full bg-white rounded-lg -mt-12 overflow-hidden">
        <div class="p-4 flex items-center justify-between gap-4">
            ...
            <div class="w-max flex items-center gap-2">
                ...
                <a href="{{ route('views.projects.create', ['client' => $client->id]) }}"
    ...
    <div class="w-full bg-white p-4 rounded-lg ">
        <table x-table x-hide="6" x-name="{{ $client->name }}-projects">
            <thead>
                <tr>
                    <td>
                        name
            ...
            <tbody>
                @foreach ($data as $row)
                    <tr>
                        ...
                        <td x-ucfirst>
                            {{ $row->manager()->first()->firstName }}
                            {{ $row->manager()->first()->lastName }}
                        </td>
                        <td>
                            {{ $row->budget }} DH
                        </td>
                        ...
                        <td>
                            {{ Carbon::parse($row->dueDate)->diffForHumans() }}
                        </td>
                        <td>
                            @include('tool-banner', [
                                'view' => route('views.projects.summary', $row->id),
                                'edit' => route('views.projects.edit', $row->id),
                                'delete' => route('actions.projects.destroy', $row->id),
                            ])
                            {{-- <div class="w-full h-full flex items-center justify-center gap-2">
                                <a href="{{ route('views.projects.summary', $row->id) }}"
                                ...
                                <a href="{{ route('views.projects.edit', $row->id) }}"
                                ...
                                <a href="{{ route('actions.projects.destroy', $row->id) }}"
