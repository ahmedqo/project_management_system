...
@section('content')
    <div class="w-full bg-white rounded-lg -mt-12 overflow-hidden">
        <div class="p-4 flex items-center justify-between gap-4">
            ...
            <div class="w-max flex items-center gap-2">
                ...
                <a href="{{ route('views.contacts.create', ['client' => $client->id]) }}"
    ...
    <div class="w-full bg-white p-4 rounded-lg ">
        <table x-table x-hide="5" x-name="{{ $client->name }}-contacts">
            <thead>
                <tr>
                    <td>
                        title
            ...
            <tbody>
                @foreach ($data as $row)
                    <tr>
                        ...
                        <td>
                            <a target="_blank" href="tel:{{ $row->phone }}" class="hover:underline focus:underline">
                                {{ $row->phone }}</a>
                        </td>
                        <td x-ucfirst>
                            {{ $row->function }}
                        ...
                        <td>
                            @include('tool-banner', [
                                'edit' => route('views.contacts.edit', $row->id),
                                'delete' => route('actions.contacts.destroy', $row->id),
                            ])
                            {{-- <div class="w-full h-full flex items-center justify-center gap-2">
                                <a href="{{ route('views.contacts.edit', $row->id) }}"
                                ...
                                <a href="{{ route('actions.contacts.destroy', $row->id) }}"
