...
@section('content')
    <div class="w-full bg-white rounded-lg -mt-12 overflow-hidden">
        <div class="p-4 flex items-center justify-between gap-4">
            ...
            <div class="w-max flex items-center gap-2">
                ...
                <a href="{{ route('views.clients.create') }}"
    ...
    <div class="w-full bg-white p-4 rounded-lg ">
        <table x-table x-hide="7" x-name="clients">
            <thead>
                <tr>
                    <td>
                        name
            ...
            <tbody>
                @foreach ($data as $row)
                    <tr>
                        ...
                        <td>
                            <a target="_blank" href="tel:{{ $row->fixPhone }}" class="hover:underline focus:underline">
                                {{ $row->fixPhone }}
                            </a>
                        </td>
                        <td>
                            <a target="_blank" href="tel:{{ $row->cellPhone }}" class="hover:underline focus:underline">
                                {{ strlen($row->cellPhone) ? $row->cellPhone : 'N/A' }}
                            </a>
                        </td>
                        <td>
                            <a target="_blank" href="tel:{{ $row->faxPhone }}" class="hover:underline focus:underline">
                                {{ strlen($row->faxPhone) ? $row->faxPhone : 'N/A' }}
                            </a>
                        </td>
                        <td>
                            <a target="_blank" href="mailto:{{ $row->email }}" class="hover:underline focus:underline">
                                {{ $row->email }}
                            </a>
                        </td>
                        <td x-ucfirst>
                        ...
                        <td>
                            @include('tool-banner', [
                                'view' => route('views.clients.summary', $row->id),
                                'edit' => route('views.clients.edit', $row->id),
                                'delete' => route('actions.clients.destroy', $row->id),
                            ])
                            {{-- <div class="w-full h-full flex items-center justify-center gap-2">
                                <a href="{{ route('views.clients.summary', $row->id) }}"
                                ...
                                <a href="{{ route('views.clients.edit', $row->id) }}"
                                ...
                                <a href="{{ route('actions.clients.destroy', $row->id) }}"
...
@section('print')
    <h1 class="text-gray-900 font-black text-2xl text-center mb-16">Clients List</h1>
        ...
        <table class="w-full text-md text-gray-900">
            <thead class="bg-primary text-xs font-black text-white uppercase">
                <tr class="uppercase">
                    ...
                    <td class="px-4 py-2">fix phone</td>
                    <td class="px-4 py-2">cell phone</td>
                    <td class="px-4 py-2">fax phone</td>
