...
@section('content')
    <div class="w-full bg-white rounded-lg -mt-12 overflow-hidden">
        <div class="p-4 flex items-center justify-between gap-4">
            ...
            <div class="w-max flex items-center gap-2">
                ...
                <a href="{{ route('views.accounts.create', ['client' => $client->id]) }}"
    ...
    <div class="w-full bg-white p-4 rounded-lg ">
        <table x-table x-hide="4" x-name="{{ $client->name }}-accounts">
            <thead>
                <tr>
                    <td>
                        bank
            ...
            <tbody>
                @foreach ($data as $row)
                    <tr>
                        ...
                        <td>
                            {{ Carbon::parse($row->account()->first()->updated_at)->diffForHumans() }}
                        </td>
                        <td>
                            @include('tool-banner', [
                                'edit' => route('views.accounts.edit', $row->account()->first()->id),
                                'delete' => route('actions.accounts.destroy', $row->account()->first()->id),
                            ])
                            {{-- <div class="w-full h-full flex items-center justify-center gap-2">
                                <a href="{{ route('views.accounts.edit', $row->account()->first()->id) }}"
                                ...
                                <a href="{{ route('actions.accounts.destroy', $row->account()->first()->id) }}"
